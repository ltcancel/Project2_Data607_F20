...
newYork <- map_data("state", region = "new york")

#create dataframe of states and group numbers
stategroups <- data.frame("BORO" = c("Bronx","Brooklyn","Manhattan","Queens","Staten Island"),
                          "group" = c(2,2,1,2,2))

#join stategroups and df data frams to add group numbers to each boro
df <- left_join(df, stategroups, by="BORO")
head(df)


ggplot() + 
  geom_polygon(data = df, aes(x=Longitude, y=Latitude, group = group), color="black", fill="lightblue")

?map_data
```

```{r}
file <- "PVS_20_v1_county_36.shp"
nymap <- st_read(file)
```
...
ggplot(df2, aes(x=state)) + geom_bar() + coord_flip()
